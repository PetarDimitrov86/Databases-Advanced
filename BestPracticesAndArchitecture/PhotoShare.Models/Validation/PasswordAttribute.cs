namespace PhotoShare.Models.Validation
{
    ...
    internal class PasswordAttribute : ValidationAttribute
    {
        ...
        public override bool IsValid(object value)
        {
            ...
            if (ContainsLowercase && !password.Any(c => char.IsLower(c)))
            {
                return false;
            }

            if (ContainsUppercase && !password.Any(c => char.IsUpper(c)))
            {
                return false;
            }

            if (ContainsDigit && !password.Any(c => char.IsDigit(c)))
            {
                return false;
            }

            if (ContainsSpecialSymbol && !password.Any(c => SpecialSymbols.Contains(c)))
            {
                return false;
            }
